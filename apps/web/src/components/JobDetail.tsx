            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="mt-5 pt-4 border-t border-gray-100">
                  <AiMatchExplanation jobId={job.id} fallbackReasons={job.explain?.topReasonsToApply ?? []} />
                </div>
...
/**
 * AI-generated "Why apply / What to watch" — pulls from the matcher's raw
 * subscores + user profile + job record through gpt-4o-mini. Lazy: only
 * fires when the user clicks "Generate". Falls back to the templated
 * subscore strings if AI isn't configured or fails.
 */
function AiMatchExplanation({ jobId, fallbackReasons }: { jobId: number; fallbackReasons: string[] }) {
  ...
  async function fire() {
    ...
    setError(null);
    try {
      const r = await apiRequest("POST", `/api/jobs/${jobId}/explain`, {});
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Why apply</h3>
          <button
            onClick={fire}
            className="inline-flex items-center gap-1 text-xs text-primary hover:underline font-medium"
            title="Generate an AI breakdown of why this match works (or doesn't) for you"
          >
            <Sparkles className="w-3 h-3" /> Get AI breakdown
          </button>
        </div>
        {fallbackReasons.length > 0 ? (
          ...
          <p className="text-xs text-gray-500">Click "Get AI breakdown" for a personalised read of this match.</p>
        )}
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-xs text-red-700 bg-red-50 border border-red-100 rounded p-2">
        {error}
        <button onClick={fire} className="ml-2 underline">Retry</button>
      </div>
    );
  }
  ...
  const verdictColors: Record<typeof data.verdict, string> = {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-start gap-2">
        <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-semibold ${verdictColors[data.verdict]}`}>
          {data.verdict} fit
        </span>
        <p className="text-sm text-gray-900 flex-1 leading-snug">{data.oneLiner}</p>
      </div>
      {data.whyApply.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Why apply</h3>
          <ul className="space-y-1">
            {data.whyApply.map((r, i) => (
              <li key={i} className="flex items-start gap-1.5 text-xs text-gray-700">
                <CheckCircle className="w-3.5 h-3.5 text-green-600 mt-0.5 shrink-0" />
                <span>{r}</span>
              </li>
      ...
      {data.whatToWatch.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">What to watch</h3>
          <ul className="space-y-1">
            {data.whatToWatch.map((r, i) => (
              <li key={i} className="flex items-start gap-1.5 text-xs text-gray-700">
      ...
      <button onClick={fire} className="text-[11px] text-primary hover:underline">Regenerate</button>
    </div>
  );
}
